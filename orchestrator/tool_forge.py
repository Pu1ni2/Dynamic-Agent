...
# Imports forbidden inside forged tools
...
_FORBIDDEN_CALLS = frozenset({
    ...
    "os.rmdir", "shutil.rmtree", "__import__",
})
...
def forge_tools_for_plan(plan: dict) -> dict[str, list[StructuredTool]]:
    ...
    # Collect unique tool specs (some agents may share tool names)
...
def _forge_single_tool(spec: dict, model: ChatOpenAI) -> StructuredTool | None:
    """Generate, validate, and wrap a single tool."""
    last_error = None
...
def _generate_code(spec: dict, model: ChatOpenAI, prev_error: str | None) -> str | None:
    """Ask the LLM to write tool code."""
    user_content = (
...
def _extract_function(namespace: dict, preferred_name: str):
    """Find the generated function in the exec namespace."""
...
def _make_safe_wrapper(func, tool_name: str):
    """Wrap a forged function so runtime errors return strings instead of raising."""
    ...
    # Copy signature and docstring so StructuredTool can inspect it
...
def _is_safe(code: str) -> tuple[bool, str]:
    """Static analysis safety check on generated code."""
    tree = ast.parse(code)
...
def _get_call_name(node: ast.Call) -> str:
    """Extract a dotted name from a Call node (best effort)."""
